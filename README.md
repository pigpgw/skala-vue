...
| ------------------------------------ | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| OpenWeatherMap 실제 날씨 데이터 적용 | 전용 Axios 인스턴스와 날씨 조회 API 함수까지 준비                         | 실제 화면과 Store 적용은 진행 전이므로 과제 6-1은 완료 처리하지 않았습니다.                                        |
| OpenWeatherMap 추가 API 연동          | 추가 API 선정 및 연동 전                                                  | 도시 좌표 변환이나 대기질 기능을 후속 구현해 과제 6-2를 확장할 예정입니다.                                         |
| 기타 외부 API 연동                    | 국토교통부 지역코드 OpenAPI와 `regionStore`를 사용해 시군구 검색 후보 제공 | 전체 50,077건 조회의 호출 부담을 확인해 첫 페이지 데이터를 우선 활용하고, 시군구 전용 데이터로 교체할 여지를 남겼습니다. |
